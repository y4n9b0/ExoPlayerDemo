...
public class DefaultTrackSelector extends MappingTrackSelector {
  ...
  public static final class ParametersBuilder extends TrackSelectionParameters.Builder {
    ...
    public ParametersBuilder setLedEnabled(boolean ledEnabled) {
  ...
  protected ExoTrackSelection.@NullableType Definition[] selectAllTracks(
    ...
    @NonNull
    List<Pair<ExoTrackSelection.Definition, Integer>> selectedTextList =
        selectTextTrack(mappedTrackInfo, rendererFormatSupports, params, selectedAudioLanguage);
    for (Pair<ExoTrackSelection.Definition, Integer> selectedText : selectedTextList) {
  ...
  protected Pair<ExoTrackSelection.Definition, Integer> selectVideoTrack(
      ...
      Parameters params)
      throws ExoPlaybackException {
    return selectBestTracksForType(
        C.TRACK_TYPE_VIDEO,
        mappedTrackInfo,
        rendererFormatSupports,
  ...
  @SuppressLint("WrongConstant") // Lint doesn't understand arrays of IntDefs.
  @NonNull
  protected List<Pair<ExoTrackSelection.Definition, Integer>> selectTextTrack(
      MappedTrackInfo mappedTrackInfo,
      @Capabilities int[][][] rendererFormatSupports,
      Parameters params,
  ...
  @Nullable
  private <T extends TrackInfo<T>> Pair<ExoTrackSelection.Definition, Integer> selectBestTracksForType(
      @C.TrackType int trackType,
      MappedTrackInfo mappedTrackInfo,
      @Capabilities int[][][] formatSupport,
      TrackInfo.Factory<T> trackInfoFactory,
      Comparator<List<T>> selectionComparator) {
    List<List<T>> selections = possibleSelectionsForType(trackType, mappedTrackInfo, formatSupport, trackInfoFactory);
    if (selections.isEmpty()) return null;
    List<T> bestSelection = max(selections, selectionComparator);
    return selection2pair(bestSelection);
  }

  @NonNull
  private <T extends TrackInfo<T>> List<Pair<ExoTrackSelection.Definition, Integer>> selectTracksForType(
      @C.TrackType int trackType,
      MappedTrackInfo mappedTrackInfo,
      @Capabilities int[][][] formatSupport,
      TrackInfo.Factory<T> trackInfoFactory) {
    List<List<T>> selections = possibleSelectionsForType(trackType, mappedTrackInfo, formatSupport, trackInfoFactory);
    List<Pair<ExoTrackSelection.Definition, Integer>> pairs = new ArrayList<>(selections.size());
    for (List<T> selection : selections) {
      pairs.add(selection2pair(selection));
    }
    return pairs;
  }

  @NonNull
  private <T extends TrackInfo<T>> List<List<T>> possibleSelectionsForType(
      @C.TrackType int trackType,
      MappedTrackInfo mappedTrackInfo,
      @Capabilities int[][][] formatSupport,
      TrackInfo.Factory<T> trackInfoFactory) {
    List<List<T>> possibleSelections = new ArrayList<>();
    int rendererCount = mappedTrackInfo.getRendererCount();
    for (int rendererIndex = 0; rendererIndex < rendererCount; rendererIndex++) {
  ...
  private <T extends TrackInfo<T>> Pair<ExoTrackSelection.Definition, Integer> selection2pair(List<T> selection) {
    int[] trackIndices = new int[selection.size()];
    for (int i = 0; i < selection.size(); i++) {
      trackIndices[i] = selection.get(i).trackIndex;
    }
    T firstTrackInfo = selection.get(0);
    ExoTrackSelection.Definition definition = new ExoTrackSelection.Definition(firstTrackInfo.trackGroup, trackIndices);
    return Pair.create(definition, firstTrackInfo.rendererIndex);
  }
