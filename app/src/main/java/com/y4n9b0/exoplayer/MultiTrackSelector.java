 ...
 *
 * <pre>{@code
 * multiTrackSelector.setParameters(
 *     multiTrackSelector.getParameters()
 *         .buildUpon()
 ...
 * }</pre>
 */
public class MultiTrackSelector extends MultiMappingTrackSelector {
    ...
    public static final class ParametersBuilder extends TrackSelectionParameters.Builder {
        ...
        private ParametersBuilder(Bundle bundle) {
            super(bundle);
            Parameters
                    defaultValue = Parameters.DEFAULT_WITHOUT_CONTEXT;
            // Video
            ...
            setExceedAudioConstraintsIfNecessary(
                    bundle.getBoolean(
                            Parameters.keyForField(
                                    Parameters.FIELD_EXCEED_AUDIO_CONSTRAINTS_IF_NCESSARY),
            ...
            setDisabledTextTrackSelectionFlags(
                    bundle.getInt(
                            Parameters.keyForField(
                                    Parameters.FIELD_DISABLED_TEXT_TRACK_SELECTION_FLAGS),
            ...
            setTunnelingEnabled(
                    bundle.getBoolean(
                            Parameters.keyForField(
                                    Parameters.FIELD_TUNNELING_ENABLED),
            ...
            rendererDisabledFlags =
                    makeSparseBooleanArrayFromTrueKeys(
                            bundle.getIntArray(
                                    Parameters.keyForField(
                                            Parameters.FIELD_RENDERER_DISABLED_INDICES)));
        }
        ...
        @Override
        public ParametersBuilder setDisabledTrackTypes(
        ...
        private void setSelectionOverridesFromBundle(Bundle bundle) {
            ...
            int[] rendererIndices =
                    bundle.getIntArray(
                            Parameters.keyForField(
                                    Parameters.FIELD_SELECTION_OVERRIDES_RENDERER_INDICES));
            List<TrackGroupArray> trackGroupArrays =
                    BundleableUtil.fromBundleNullableList(
                            TrackGroupArray.CREATOR,
                            bundle.getParcelableArrayList(
                                    Parameters.keyForField(
                                            Parameters.FIELD_SELECTION_OVERRIDES_TRACK_GROUP_ARRAYS)),
            ...
            SparseArray<SelectionOverride> selectionOverrides =
                    BundleableUtil.fromBundleNullableSparseArray(
                            ...
                            bundle.getSparseParcelableArray(
                                    Parameters.keyForField(
                                            Parameters.FIELD_SELECTION_OVERRIDES)),
    ...
    /**
     * Extends {@link Parameters} by adding fields that are specific to {@link MultiTrackSelector}.
     */
    ...
    private final AtomicReference<Parameters>
            parametersReference;
     ...
     */
    public MultiTrackSelector(Context context, ExoTrackSelection.Factory trackSelectionFactory) {
    ...
    selectTracks(
        ...
        ExoTrackSelection.Definition[] definitions =
                selectAllTracks(
                        ...
                        params);
        Log.e("Bob", "definitions.size=" + definitions.length);

        // Apply per track type overrides.
        SparseArray<Pair<TrackSelectionOverrides.TrackSelectionOverride, Integer>> applicableOverridesByTrackType =
                getApplicableOverrides(mappedTrackInfo, params);
        for (int i = 0; i < applicableOverridesByTrackType.size(); i++) {
            Pair<TrackSelectionOverrides.TrackSelectionOverride, Integer> overrideAndRendererIndex =
        ...
        for (int i = 0; i < rendererCount; i++) {
            if (hasLegacyRendererOverride(mappedTrackInfo, params, /* rendererIndex= */ i)) {
                Log.e("Bob", "hasLegacyRendererOverride definitions[" + i + "]");
        ...
        for (int i = 0; i < rendererCount; i++) {
            if (isRendererDisabled(mappedTrackInfo, params, /* rendererIndex= */ i)) {
                definitions[i] = null;
                Log.e("Bob", "Disable renderers definitions[" + i + "]=null");
            }
        }
        ...
        for (int i = 0; i < rendererCount; i++) {
            ...
            boolean isTrackTypeNone = mappedTrackInfo.getRendererType(i) == C.TRACK_TYPE_NONE;
            boolean isTrackSelectionNull = rendererTrackSelections[i] == null;
            Log.e("Bob", "i=" + i + ", rendererType=" + rendererType + ", rendererEnabled=" + rendererEnabled
                    + ", forceRendererDisabled="
                    + forceRendererDisabled + ", "
                    + "isTrackTypeNone=" + isTrackTypeNone + ", isTrackSelectionNull=" + isTrackSelectionNull);
            if (!rendererEnabled) {
                Log.e("Bob", "Disable renderers rendererConfigurations[" + i + "]=null");
            }
        }
    ...
    private ExoTrackSelection.Definition getLegacyRendererOverride(
        ...
        if (override == null) {
            Log.e("Bob", "getLegacyRendererOverride definitions[" + rendererIndex + "] = null");
            return null;
        }
     ...
     * for this override.
     */
    private SparseArray<Pair<TrackSelectionOverrides.TrackSelectionOverride, Integer>> getApplicableOverrides(
            MappedTrackInfo mappedTrackInfo, Parameters params) {
        SparseArray<Pair<TrackSelectionOverrides.TrackSelectionOverride, Integer>> applicableOverrides =
                new SparseArray<>();
    ...
    private void maybeUpdateApplicableOverrides(
            SparseArray<Pair<TrackSelectionOverrides.TrackSelectionOverride, Integer>> applicableOverrides,
            @Nullable TrackSelectionOverrides.TrackSelectionOverride override,
            int rendererIndex) {
        if (override == null) {
            return;
        }
        ...
        @Nullable
        Pair<TrackSelectionOverrides.TrackSelectionOverride, Integer> existingOverride = applicableOverrides.get(
                trackType);
    ...
    private void applyTrackTypeOverride(
        ...
        for (int i = 0; i < definitions.length; i++) {
            if (overrideRendererIndex == i) {
                ...
                definitions[i] = null;
                Log.e("Bob", "applyTrackTypeOverride definitions[" + i + "] = null");
            }
        }
    }
    ...
    /**
     * Called from {@link #selectTracks(MappedTrackInfo, int[][][], int[], MediaSource.MediaPeriodId, Timeline)}
     ...
     * @param rendererFormatSupports The {@link RendererCapabilities.Capabilities} for each mapped track, indexed by
     *     renderer, track group and track (in that order).
     * @param mixedMimeTypeSupports The {@link RendererCapabilities.AdaptiveSupport} for mixed MIME type adaptation
     *                              for the
     *     renderer.
     ...
     * @param formatSupport The {@link RendererCapabilities.Capabilities} for each mapped track, indexed by track
     *                      group and
     *     track (in that order).
     * @param params The selector's current constraint parameters.
    ...
    protected ExoTrackSelection.Definition selectOtherTrack(
            int trackType, TrackGroupArray groups, @RendererCapabilities.Capabilities int[][] formatSupport,
            Parameters params)
            throws ExoPlaybackException {
        ...
        for (int groupIndex = 0; groupIndex < groups.length; groupIndex++) {
            ...
            for (int trackIndex = 0; trackIndex < trackGroup.length; trackIndex++) {
                if (isSupported(
                    ...
                    OtherTrackScore
                            trackScore = new OtherTrackScore(format, trackFormatSupport[trackIndex]);
     ...
     *
     * @param mappedTrackInfo Mapped track information.
     * @param renderererFormatSupports The {@link RendererCapabilities.Capabilities} for each mapped track, indexed by
     *     renderer, track group and track (in that order).
    ...
    private static void maybeConfigureRenderersForTunneling(
            MappedTrackInfo mappedTrackInfo,
            @RendererCapabilities.Capabilities int[][][] renderererFormatSupports,
     ...
     *
     * @param formatSupport The {@link RendererCapabilities.Capabilities} for each track, indexed by group index and
     *                      track
     *     index (in that order).
    ...
    /**
     * Returns true if the {@link C.FormatSupport} in the given {@link RendererCapabilities.Capabilities} is {@link
     * C#FORMAT_HANDLED} or if {@code allowExceedsCapabilities} is set and the format support is
     * {@link C#FORMAT_EXCEEDS_CAPABILITIES}.
     *
     * @param formatSupport {@link RendererCapabilities.Capabilities}.
    ...
    protected static boolean isSupported(
            @RendererCapabilities.Capabilities int formatSupport, boolean allowExceedsCapabilities) {
    ...
    private abstract static class TrackInfo<T extends TrackInfo<T>> {
        ...
        public interface Factory<T extends TrackInfo<T>> {
            List<T> create(int rendererIndex, TrackGroup trackGroup,
                    @RendererCapabilities.Capabilities int[] formatSupports);
        }
    ...
    private static final class VideoTrackInfo extends
        ...
        public static ImmutableList<VideoTrackInfo> createForTrackGroup(
                ...
                Parameters params,
                @RendererCapabilities.Capabilities int[] formatSupport,
                @RendererCapabilities.AdaptiveSupport int mixedMimeTypeAdaptionSupport) {
        ...
        public VideoTrackInfo(
                ...
                Parameters parameters,
                @RendererCapabilities.Capabilities int formatSupport,
                @RendererCapabilities.AdaptiveSupport int mixedMimeTypeAdaptationSupport,
                boolean isSuitableForViewport) {
        ...
        private @SelectionEligibility int evaluateSelectionEligibility(
                @RendererCapabilities.Capabilities int rendererSupport,
                @RendererCapabilities.AdaptiveSupport int requiredAdaptiveSupport) {
        ...
        private static int compareNonQualityPreferences(
                VideoTrackInfo info1, VideoTrackInfo info2) {
        ...
        private static int compareQualityPreferences(
                VideoTrackInfo info1, VideoTrackInfo info2) {
    ...
    private static final class AudioTrackInfo extends TrackInfo<AudioTrackInfo>
        ...
        private @SelectionEligibility int evaluateSelectionEligibility(
                @RendererCapabilities.Capabilities int rendererSupport, boolean hasMappedVideoTracks) {
            if (!isSupported(rendererSupport, parameters.exceedRendererCapabilitiesIfNecessary)) {
                return SELECTION_ELIGIBILITY_NO;
            }
    ...
    private static final class TextTrackInfo extends TrackInfo<TextTrackInfo>
        ...
        public TextTrackInfo(
                ...
                int trackIndex,
                Parameters parameters,
                @RendererCapabilities.Capabilities int trackFormatSupport,
                @Nullable String selectedAudioLanguage) {
            super(rendererIndex, trackGroup, trackIndex);
            isWithinRendererCapabilities =
    ...
    private static final class OtherTrackScore implements Comparable<OtherTrackScore> {
        ...
        public OtherTrackScore(Format format, @RendererCapabilities.Capabilities int trackFormatSupport) {
